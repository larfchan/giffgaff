...
一个极简 Android 工具：用户手动点击后，经**默认移动数据 SIM 的蜂窝网络**发送**一个 1 字节 UDP 包**。没有广告、分析 SDK、后台服务、定时任务或自有服务器。

> 这不是“严格只产生 1KB 流量”的承诺。APK 能控制的是自己的单个 UDP 包；Android 拉起/验证蜂窝网络、DNS、蜂窝承载开销和运营商计费取整不受应用控制。
...
1. 在 Android 设置中，把 giffgaff 设为**默认移动数据 SIM**。
...
Stock Android 没有标准的“只给某个 APK 数据漫游权限”。本应用申请的是一个带互联网能力的蜂窝网络；系统的数据漫游开关和厂商的按应用联网策略仍然有效，应用不会绕过它们。

Android 公共 API 也不能让普通应用可靠地选中任意非默认 SIM。因此：

- 必须先将 giffgaff 设为默认移动数据 SIM；
- 默认启用“仅在系统报告正在漫游时发送”；
...
首次使用后请查看 giffgaff 的使用记录或余额变化进行核验。在确认这种方式对自己的 SIM 有效之前，保留一次合格短信作为兜底；也建议约每 5 个月操作一次，不要等到六个月期限当天。
...
官方公开页面没有说明漫游数据的最小计费单位、会话最低扣款或具体取整方式，所以不能用“1 字节负载”推导出精确账单金额。普通套餐在中国等 EU 区域以外通常不包含漫游数据，需要 Airtime Credit；价格也可能变化，使用前请重新查看官方页面。
...
Debug APK 可直接侧载，但不同 GitHub Actions 运行生成的 debug 签名可能不同，因此新版不一定能覆盖安装旧版。长期使用应配置由 GitHub Secrets 提供的固定 release keystore；私钥不要提交到仓库。
...
设置仅保存在设备本地。应用启动时不联网，只有用户点击按钮并通过确认项后才发送。
